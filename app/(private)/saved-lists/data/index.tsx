...
export const SavedListPage = () => {
  ...
  const { data: allListsResponse } = useUserLists(userId)
  const allUserLists: SavedList[] = allListsResponse?.lists ?? []
  const {
    data: displayedListsResponse,
    isLoading: isDisplayLoading,
    isError,
  } = useUserLists(userId, activeTab)
  const displayedLists: SavedList[] = displayedListsResponse?.lists ?? []
  console.log('allListRepsonse', allListsResponse)

  const { mutateAsync: updateUserList, isPending: isUpdating } = useUpdateUserList()

  const isAllLoading = isAuthLoading || isDisplayLoading
